 ...
 * utilizzando e poi senza utilizzare l'istruzione for. Messaggio di errore se la somma va in overflow. 
 *
*/
#include <iostream>
using namespace std; 
int main (){
    ...
    int s = 0; // somma dei valori; 
    int numero;
    int max = 0; 
    for (int i=1; i<=n; i++){
        cout << "Inserisci un numero intero positivo "; 
        cin>>numero; 
        s+=numero; // s=s+numero, ed è la somma iterativa dei numeri. 
        if (numero>max)
            max = numero; 
        // devostare attenta ad inizializzare max a 0, perchè se voglio  guardare il massimo tra numeri positivi max=0 va bene, perchè tutti i numeri che prendo dovrebbero essere più grandi di zero. ma se voglio guardare al valore massimo di numeri che potrebbero essere negativi, devo iniziializzare max al più grande valore negativo possibile (min_int). 
    }
    cout << "s" << "=" << s << endl; 
    if (numero>(max_int-=s)) // s+=numero>max_int, ma così non lo posso scrivere perchè non posso fare il controllo, i valori più alti di max_int sono casuali, per poter fare il controllo devo essere ancora negli interi possibili. e quindi lo porto di là e diventa negativo. 
